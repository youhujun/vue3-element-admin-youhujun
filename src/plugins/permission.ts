 ...
 * @LastEditTime: 2025-08-17 16:06:42
 * @FilePath: \src\plugins\permission.ts
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    try {
      ...
      if (!permissionStore.isDynamicRoutesGenerated) {
        // 将 userInfo 断言为我们定义的 UserInfo 类型
        const userInfo = userStore.userInfo as AdminUserInfo;
...
export function hasAuth(value: string | string[], type: "button" | "role" = "button") {
  ...
  // 超级管理员 拥有所有权限
  if (type === "button" && (roles.includes("develop") || roles.includes("super"))) {
    return true;
  }
