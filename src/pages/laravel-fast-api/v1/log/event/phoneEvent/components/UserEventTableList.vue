 ...
 * @LastEditTime: 2025-12-05 16:20:02
 * @FilePath: \src\pages\laravel-fast-api\v1\log\event\phoneEvent\components\UserEventTableList.vue
    ...
    >
      ...
      <el-table-column label="详情" type="expand" align="center" fixed>
        <template #default="scope">
          <UserEventListItemDetails
            v-model:row-data="scope.row"
            v-model:show-index="showIndex"
          ></UserEventListItemDetails>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="event_name" label="事件名称" min-width="200" />
      <el-table-column align="center" prop="event_code" label="事件编码" min-width="300" />
      <el-table-column align="center" prop="event_type" label="类型编号" min-width="100" />
...
//组件
import UserEventListItemDetails from "./UserEventListItemDetails.vue";

//api和type
import UserEventAPI from "@/api/laravel-fast-api/v1/log/event/phone/user-event-log-api";
import type { UserEventLogListItem } from "@/api/laravel-fast-api/v1/log/event/phone/user-event-log-type";

//定义组件
defineOptions({
  name: "UserEventTableList",
});
...
//选中对象的数组
const multipleSelection = ref<UserEventLogListItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: UserEventLogListItem) => {
...
//定义数据列表
const dataList = defineModel<UserEventLogListItem[]>("dataList", {
  default: () => [],
});
...
//删除
const handleToDelete = (index: number, row: UserEventLogListItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await UserEventAPI.deleteUserEventLog(param);
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        UserEventAPI.multipleDeleteUserEventLog(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
