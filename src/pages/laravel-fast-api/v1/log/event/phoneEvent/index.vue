 ...
 * @LastEditTime: 2025-12-10 04:02:46
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\log\event\phoneEvent\index.vue
...
<template>
  <div>
    <!-- 页面内容开始 -->
    <YhCard>
      <template #cardTitle>用户事件日志</template>
      <template #cardNotice>
      ...
      <template #cardContent>
        ...
        <YhTableLayout>
          ...
          <template #search>
            ...
            <UserEventSearch @update:handle-change-search="toSearch"></UserEventSearch>
          </template>
          <!-- 搜索区结束 -->
          <!-- 批量操作区开始 -->
          <template #batch>
...
//页面级组件
import UserEventSearch from "./components/UserEventSearch.vue";
import UserEventTableList from "./components/UserEventTableList.vue";
...
const getUserEventList = async () => {
  const result = await UserEventAPI.getUserEventLog(where.value);
