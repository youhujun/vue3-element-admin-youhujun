 ...
 * @LastEditTime: 2025-12-10 03:56:28
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\log\event\adminEvent\components\AdminEventTableList.vue
...
//组件
import AdminEventListItemDetails from "./AdminEventListItemDetails.vue";
...
//选中对象的数组
const multipleSelection = ref<AdminEventLogListItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: AdminEventLogListItem) => {
...
//定义数据列表
const dataList = defineModel<AdminEventLogListItem[]>("dataList", {
  default: () => [],
});
...
//删除
const handleToDelete = (index: number, row: AdminEventLogListItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await AdminEventAPI.deleteAdminEventLog(param);
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        AdminEventAPI.multipleDeleteAdminEventLog(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
