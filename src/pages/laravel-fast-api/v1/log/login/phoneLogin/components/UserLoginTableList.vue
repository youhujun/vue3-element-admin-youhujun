 ...
 * @LastEditTime: 2025-12-10 03:43:14
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\log\login\phoneLogin\components\UserLoginTableList.vue
    ...
    >
      ...
      <el-table-column label="详情" type="expand" align="center" fixed>
        <template #default="scope">
          <UserLoginListItemDetails
            v-model:row-data="scope.row"
            v-model:show-index="showIndex"
          ></UserLoginListItemDetails>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="ip" label="ip地址" min-width="200" />
      <el-table-column align="center" prop="instruction" label="说明" min-width="300" />
      <el-table-column align="center" prop="status" label="类型" min-width="100">
        <template #default="scope">
          <el-tag v-show="scope.row.status == 10" type="success">登录</el-tag>
          <el-tag v-show="scope.row.status == 20" type="warning">退出</el-tag>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="phone" label="手机号" min-width="300" />
      <el-table-column align="center" prop="account_name" label="账号" min-width="300" />
...
//组件
import UserLoginListItemDetails from "./UserLoginListItemDetails.vue";

//api和type
import UserLoginAPI from "@/api/laravel-fast-api/v1/log/login/phone/user-login-log-api";
import type { UserLoginLogListItem } from "@/api/laravel-fast-api/v1/log/login/phone/user-login-log-type";

//定义组件
defineOptions({
  name: "UserLoginTableList",
});
...
//选中对象的数组
const multipleSelection = ref<UserLoginLogListItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: UserLoginLogListItem) => {
...
//定义数据列表
const dataList = defineModel<UserLoginLogListItem[]>("dataList", {
  default: () => [],
});
...
// 获取表格实例（需与模板中ref对应）
const adminEventTable = ref<TableInstance | null>(null);

//删除
const handleToDelete = (index: number, row: UserLoginLogListItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await UserLoginAPI.deleteUserLoginLog(param);
...
watch(
  ...
  (newValue) => {
    ...
    // 执行反选操作
    adminEventTable.value?.clearSelection();
    diffIndex.value.forEach((index) => {
      const row = dataList.value[index];
      if (row) {
        adminEventTable.value?.toggleRowSelection(row, true);
      }
    });
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        UserLoginAPI.multipleDeleteUserLoginLog(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
