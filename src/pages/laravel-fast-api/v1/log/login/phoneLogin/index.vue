 ...
 * @LastEditTime: 2025-12-10 02:41:32
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\log\login\phoneLogin\index.vue
...
<template>
  <div>
    <!-- 页面内容开始 -->
    <YhCard>
      <template #cardTitle>用户登录日志</template>
      <template #cardNotice>
      ...
      <template #cardContent>
        ...
        <YhTableLayout>
          ...
          <template #search>
            ...
            <UserLoginSearch @update:handle-change-search="toSearch"></UserLoginSearch>
          </template>
          <!-- 搜索区结束 -->
          <!-- 批量操作区开始 -->
          <template #batch>
...
//页面级组件
import UserLoginSearch from "./components/UserLoginSearch.vue";
import UserLoginTableList from "./components/UserLoginTableList.vue";

//store
import { useUserStore } from "@/store";
//api和type
import UserLoginLogAPI from "@/api/laravel-fast-api/v1/log/login/phone/user-login-log-api";
//store
...
const initPageWhere = () => {
  ...
  where.value.user_id = 0;
  where.value.status = 0;
};

//获取列表
const getUserLoginList = async () => {
  const result = await UserLoginLogAPI.getUserLoginLog(where.value);
