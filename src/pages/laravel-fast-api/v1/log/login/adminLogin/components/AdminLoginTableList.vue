 ...
 * @LastEditTime: 2025-12-10 03:58:12
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\log\login\adminLogin\components\AdminLoginTableList.vue
...
//组件
import AdminLoginListItemDetails from "./AdminLoginListItemDetails.vue";

//api和type
import AdminLoginAPI from "@/api/laravel-fast-api/v1/log/login/admin/admin-login-log-api";
import type { AdminLoginLogListItem } from "@/api/laravel-fast-api/v1/log/login/admin/admin-login-log-type";

//定义组件
defineOptions({
  name: "AdminLoginTableList",
});
...
//选中对象的数组
const multipleSelection = ref<AdminLoginLogListItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: AdminLoginLogListItem) => {
...
//定义数据列表
const dataList = defineModel<AdminLoginLogListItem[]>("dataList", {
  default: () => [],
});
...
//删除
const handleToDelete = (index: number, row: AdminLoginLogListItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await AdminLoginAPI.deleteAdminLoginLog(param);
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        AdminLoginAPI.multipleDeleteAdminLoginLog(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
