 ...
 * @LastEditTime: 2025-12-10 03:58:27
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\log\login\adminLogin\index.vue
...
<template>
  <div>
    <!-- 页面内容开始 -->
    <YhCard>
      <template #cardTitle>后台登录日志</template>
      <template #cardNotice>
      ...
      <template #cardContent>
        ...
        <YhTableLayout>
          ...
          <template #search>
            ...
            <AdminLoginSearch @update:handle-change-search="toSearch"></AdminLoginSearch>
          </template>
          <!-- 搜索区结束 -->
          <!-- 批量操作区开始 -->
          <template #batch>
...
//页面级组件
import AdminLoginSearch from "./components/AdminLoginSearch.vue";
import AdminLoginTableList from "./components/AdminLoginTableList.vue";

//store
import { useUserStore } from "@/store";
//api和type
import AdminLoginLogAPI from "@/api/laravel-fast-api/v1/log/login/admin/admin-login-log-api";
//store
...
//获取列表
const getAdminLoginList = async () => {
  const result = await AdminLoginLogAPI.getAdminLoginLog(where.value);
