<template>
  <div>
    ...
    >
      <el-row type="flex" justify="center">
        <el-col :span="24">
          <el-form-item label="名称" prop="replace_name">
            <el-input
              v-model="createForm.replace_name"
              placeholder="请输入名称"
              clearable
              class="yh-input"
            />
      ...
      <el-row type="flex" justify="center">
        <el-col :span="24">
          <el-form-item label="值" prop="replace_value">
            <el-input
              v-model="createForm.replace_value"
              placeholder="请输入值"
              clearable
              class="yh-input"
            />
      ...
      >
        <el-col :span="24">
          <el-form-item label="配置项路径" prop="replace_path">
            <YhUploadFile
              v-model:file-path="createForm.replace_path"
              file-name="file"
      ...
      <el-row v-show="createForm.type == 10" type="flex" justify="center">
        <el-col :span="24">
          <el-form-item label="本地地址" prop="replace_path">
            <el-input v-model="createForm.replace_path" disabled class="yh-input" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row v-show="createForm.type == 20" type="flex" justify="center">
        <el-col :span="24">
      ...
      <el-row type="flex" justify="center">
        ...
        <el-col :span="12">
          <el-form-item label="时间" prop="time">
            <FutureSelectDateTime v-model:future-time="createForm.time"></FutureSelectDateTime>
          </el-form-item>
...
//api和type
import type { AddReplace } from "@/api/laravel-fast-api/v1/replace/replace/replace-type";
import ReplaceAPI from "@/api/laravel-fast-api/v1/replace/replace/replace-api";

//定义组件名称
defineOptions({
  name: "CreateReplaceForm",
});

const createFormRef = ref<FormInstance>();
const createForm = reactive<AddReplace>({
  replace_name: "",
  replace_value: "",
  type: 10,
  replace_path: "",
...
const createRules = reactive<FormRules<AddReplace>>({
...
const initCreateForm = () => {
  createForm.replace_name = "";
  createForm.replace_value = "";
  createForm.type = 10;
  createForm.replace_path = "";
  createForm.replace_url = "";
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    if (valid) {
      ...
      const result = await ReplaceAPI.addReplace(createForm);
      // 3. 提交成功提示
