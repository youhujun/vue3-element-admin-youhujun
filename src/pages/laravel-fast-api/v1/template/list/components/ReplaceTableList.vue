 ...
 * @LastEditTime: 2025-12-10 04:47:25
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\template\list\components\ReplaceTableList.vue
...
<template>
  <div>
    ...
    >
      ...
      <el-table-column align="center" prop="replace_name" label="名称" min-width="300" />
      <el-table-column align="center" prop="replace_value" label="值" min-width="300" />
...
//组件
import ReplaceListItemDetails from "./ReplaceListItemDetails.vue";
...
//
const beforeReplaceForm = defineModel<ReplaceItem>("beforeReplaceForm", {
  default: () => {
    return {
      id: 0,
      replace_name: "",
...
//选中对象的数组
const multipleSelection = ref<ReplaceItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: ReplaceItem) => {
...
//定义数据列表
const dataList = defineModel<ReplaceItem[]>("dataList", {
  default: () => [],
});
...
//类型
const filterType = (value: string, row: ReplaceItem) => {
  return Number(row.type) == Number(value);
};
//编辑
const handleToEditPage = (index: number, row: ReplaceItem) => {
  //复杂情景跳转页面
  //console.log(row)
  ...
  dialogType.value = 20;
  beforeReplaceForm.value = row;
};

//删除
const handleToDelete = (index: number, row: ReplaceItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await ReplaceAPI.deleteReplace(param);
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        ReplaceAPI.multipleDeleteReplace(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
