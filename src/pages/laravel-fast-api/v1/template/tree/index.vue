 ...
 * @LastEditTime: 2025-12-10 04:59:34
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\template\tree\index.vue
...
//页面级组件
import CreateReplaceTreeForm from "./components/CreateReplaceTreeForm.vue";
import UpdateReplaceTreeForm from "./components/UpdateReplaceTreeForm.vue";
import ReplaceRowData from "./components/ReplaceRowData.vue";
...
//源数据
const soureTreeData = ref<ReplaceItem[]>([]);
...
//创建表单
const propsCreateForm = ref<AddReplace>({
  id: 0,
  parent_id: 0,
  deep: 1,
  replace_name: "",
...
//查看单挑数据
const propsRowData = ref<ReplaceItem>({
  id: 0,
  parent_id: 0,
  deep: 1,
  replace_name: "",
 ...
 * 处理添加下级
 */
const handleAddChild = (node: TreeNode, data: ReplaceItem) => {
 ...
 * 更新
 */
const handleToUpdate = (node: TreeNode, data: ReplaceItem) => {
  //console.log('data',data)
 ...
 * 删除
 */
const handleToRemove = (node: TreeNode, data: ReplaceItem) => {
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await ReplaceAPI.deleteReplace(data);
        getTreeReplace();
 ...
 * 查看详情
 */
const handleToShow = (node: TreeNode, data: ReplaceItem) => {
  //初始化查看详情
  initPropsRowData();
...
const handleDrop = debounce(function (draggingNode: Node, dropNode: Node, dropType: NodeDropType) {
  ...
  ReplaceAPI.moveReplace(dragParam.value).then(() => {
    //console.log(res)
    //初始化拖拽参数
    initDragParam();
    //重新加载
...
//获取服务器树形路由
const getTreeReplace = async () => {
  const result = await ReplaceAPI.getTreeReplace();
  const { data } = result;
...
const initPropsUpdateForm = () => {
  ...
  propsUpdateForm.value.replace_name = "";
...
const initPropsRowData = () => {
  ...
  propsRowData.value.replace_name = "";
