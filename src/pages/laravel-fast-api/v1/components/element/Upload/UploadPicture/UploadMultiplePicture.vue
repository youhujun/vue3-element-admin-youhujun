<template>
  ...
  >
    <template #showContent>
      <!-- 选择相册 -->
      <SelectAlbum
        v-if="isShowSelectAlbum"
        v-model:select-album-id="uploadAlbumId"
        v-has-role="['develop', 'super', 'album_admin']"
      ></SelectAlbum>
      <!-- 选择相册结束 -->
      <el-divider v-if="isShowSelectAlbum">
...
import { BusUploadMultiplePicture } from "@/api/laravel-fast-api/v1/picture/picture/picture-type";
...
const submitUpload = async () => {
  ...
  //通知上传图片完成
  bus.emit("uploadMultiplePictureFinish", true);
  //初始化相册id
  uploadAlbumId.value = 0;
};

//事件监听函数
const handleToUploadMultiplePicture = (param: BusUploadMultiplePicture) => {
  //console.log('上传多图')
  //console.log('传递的参数param')
  //console.log(param)
  dialogVisible.value = true;
  dialogType.value = 30;
  dialogTitle.value = "上传图片";
  //上传图片的相册id
  uploadAlbumId.value = param.uploadAlbumId;
...
//监听上传多图
bus.on("uploadMultiplePicture", handleToUploadMultiplePicture);

// 组件卸载时移除监听,防止内存泄漏
onUnmounted(() => {
  bus.off("uploadMultiplePicture", handleToUploadMultiplePicture);
  isShowSelectAlbum.value = true;
  //初始化相册id
  uploadAlbumId.value = 0;
});
