...
//api和type
import type { AddLevelItem } from "@/api/laravel-fast-api/v1/business/level/item/level-item-type";
import LevelItemAPI from "@/api/laravel-fast-api/v1/business/level/item/level-item-api";

//定义组件名称
defineOptions({
  name: "CreateLevelItemForm",
});

const createFormRef = ref<FormInstance>();
const createForm = reactive<AddLevelItem>({
  item_name: "",
  item_code: "",
  type: 10,
  description: "",
...
const createRules = reactive<FormRules<AddLevelItem>>({
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    if (valid) {
      ...
      const result = await LevelItemAPI.addLevelItem(createForm);
      // 3. 提交成功提示
