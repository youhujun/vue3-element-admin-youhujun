 ...
 * @LastEditTime: 2025-12-10 04:22:07
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\business\level\levelItem\index.vue
...
<template>
  <div>
    <!-- 页面内容开始 -->
    <YhCard>
      <template #cardTitle>级别条件</template>
      <template #cardNotice>
        <el-tag size="small" type="danger" effect="plain">这是配置是级别配置的基础</el-tag>
      </template>
      <template #cardButton>
      ...
      <template #cardContent>
        ...
        <YhTableLayout>
          ...
          <template #search>
            ...
            <LevelItemSearch @update:handle-change-search="toSearch"></LevelItemSearch>
          </template>
          <!-- 搜索区结束 -->
          <!-- 批量操作区开始 -->
          <template #batch>
          ...
          <template #table>
            ...
            <LevelItemTableList
              ...
              v-model:before-level-item-form="beforeLevelItemForm"
              :loading="loading"
...
//页面级组件
import LevelItemSearch from "./components/LevelItemSearch.vue";
import LevelItemTableList from "./components/LevelItemTableList.vue";
import CreateLevelItemForm from "./components/CreateLevelItemForm.vue";
import UpdateLevelItemForm from "./components/UpdateLevelItemForm.vue";
//store
...
//修改用户的容器
const beforeLevelItemForm = ref<LevelItem>({
  id: 0,
  item_name: "",
  item_code: "",
  type: 10,
...
//监听beforeLevelItemForm的变化
watch(
  () => beforeLevelItemForm.value,
  (newValue) => {
...
//修改表单
const propsUpdateForm = ref<UpdateLevelItem>({
  id: 0,
  item_name: "",
  item_code: "",
  type: 10,
...
//获取列表
const getLevelItemList = async () => {
  const result = await LevelItemAPI.getLevelItem(where.value);
