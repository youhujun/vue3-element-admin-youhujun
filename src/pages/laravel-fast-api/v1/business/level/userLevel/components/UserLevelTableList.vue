 ...
 * @LastEditTime: 2025-12-10 04:19:26
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\business\level\userLevel\components\UserLevelTableList.vue
    ...
    >
      ...
      <el-table-column label="详情" type="expand" align="center" fixed>
        <template #default="scope">
          <UserLevelListItemDetails
            v-model:row-data="scope.row"
            v-model:show-index="showIndex"
          ></UserLevelListItemDetails>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="level_name" label="级别名称" min-width="300" />
      <el-table-column align="center" prop="level_code" label="级别编码" min-width="300" />
      <el-table-column align="center" prop="amount" label="金额" min-width="300" />
...
//组件
import UserLevelListItemDetails from "./UserLevelListItemDetails.vue";
...
//
const beforeUserLevelForm = defineModel<UserLevelListItem>("beforeUserLevelForm", {
  default: () => {
    return {
      id: 0,
      level_name: "",
...
//选中对象的数组
const multipleSelection = ref<UserLevelListItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: UserLevelListItem) => {
...
//定义数据列表
const dataList = defineModel<UserLevelListItem[]>("dataList", {
  default: () => [],
});
...
// 获取表格实例（需与模板中ref对应）
const userLevelTable = ref<TableInstance | null>(null);
//编辑
const handleToEditPage = (index: number, row: UserLevelListItem) => {
  //复杂情景跳转页面
  //console.log(row)
  ...
  dialogTitle.value = "编辑用户级别";
  dialogType.value = 20;
  beforeUserLevelForm.value = row;
};

//删除
const handleToDelete = (index: number, row: UserLevelListItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await UserLevelAPI.deleteUserLevel(param);
...
watch(
  ...
  (newValue) => {
    ...
    // 执行反选操作
    userLevelTable.value?.clearSelection();
    diffIndex.value.forEach((index) => {
      const row = dataList.value[index];
      if (row) {
        userLevelTable.value?.toggleRowSelection(row, true);
      }
    });
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        UserLevelAPI.multipleDeleteUserLevel(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
