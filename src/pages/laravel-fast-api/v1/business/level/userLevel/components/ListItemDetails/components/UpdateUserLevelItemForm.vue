      ...
      <el-row type="flex" justify="center">
        <el-col :span="24">
          <el-form-item label="配置项" prop="level_item_id">
            <SelectLevelItem
              v-model:select-level-item-id="updateForm.level_item_id"
            ></SelectLevelItem>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row type="flex" justify="center">
        <el-col :span="24">
          <el-form-item label="配置项与值的关系" prop="value_type">
            <el-select v-model="updateForm.value_type" placeholder="请选择" class="select">
              <el-option
...
//定义组件名称
defineOptions({
  name: "UpdateUserLevelItemForm",
});

//定义props
const props = withDefaults(
  defineProps<{
    propsUpdateForm?: UpdateUserLevelItemUnion;
    userLevelId?: number;
  }>(),
  {
    propsUpdateForm: () => ({
...
const updateForm = reactive<UpdateUserLevelItemUnion>({
  id: 0,
  user_level_id: 0,
  level_item_id: 0,
  value: "",
...
const updateRules = reactive<FormRules<UpdateUserLevelItemUnion>>({
  level_item_id: [{ type: "number", required: true, message: "必须有配置项", trigger: "blur" }],
  value: [{ required: true, message: "必须有值", trigger: "blur" }],
});
...
const initUpdateForm = () => {
  updateForm.id = 0;
  updateForm.user_level_id = 0;
  updateForm.level_item_id = 0;
  updateForm.value = "";
  updateForm.value_type = 10;
  updateForm.sort = 100;
};
...
watch(
  ...
  (newValue) => {
    ...
    updateForm.user_level_id = newValue; // 例如同步到表单中
  },
  { immediate: true }
);
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    if (valid) {
      ...
      const result = await UserLevelItemUnionAPI.updateUserLevelItemUnion(updateForm);
      // 3. 提交成功提示
