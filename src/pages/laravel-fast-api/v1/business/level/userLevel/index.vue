 ...
 * @LastEditTime: 2025-12-10 02:20:19
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\business\level\userLevel\index.vue
...
<template>
  <div>
    <!-- 页面内容开始 -->
    <YhCard>
      <template #cardTitle>用户级别</template>
      <template #cardNotice>
        <el-tag size="small" type="danger" effect="plain">级别配置项请在详情里查看</el-tag>
      </template>
      <template #cardButton>
      ...
      <template #cardContent>
        ...
        <YhTableLayout>
          ...
          <template #search>
            ...
            <UserLevelSearch @update:handle-change-search="toSearch"></UserLevelSearch>
          </template>
          <!-- 搜索区结束 -->
          <!-- 批量操作区开始 -->
          <template #batch>
          ...
          <template #table>
            ...
            <UserLevelTableList
              ...
              v-model:before-user-level-form="beforeUserLevelForm"
              :loading="loading"
...
//页面级组件
import UserLevelSearch from "./components/UserLevelSearch.vue";
import UserLevelTableList from "./components/UserLevelTableList.vue";
import CreateUserLevelForm from "./components/CreateUserLevelForm.vue";
import UpdateUserLevelForm from "./components/UpdateUserLevelForm.vue";
//store
...
//修改用户的容器
const beforeUserLevelForm = ref<UserLevelListItem>({
  id: 0,
  level_name: "",
  level_code: "",
  amount: 10,
...
//监听beforeUserLevelForm的变化
watch(
  () => beforeUserLevelForm.value,
  (newValue) => {
...
//修改表单
const propsUpdateForm = ref<UpdateUserLevel>({
  id: 0,
  level_name: "",
  level_code: "",
  amount: 0,
...
const getUserLevelList = async () => {
  const result = await UserLevelAPI.getUserLevel(where.value);
...
//重新加载
const listenToReloadUserLevelList = () => {
  loading.value = true;
  //重新获取用户列表
  getUserLevelList();
};
...
//监听重新加载
bus.on("reloadUserLevelList", listenToReloadUserLevelList);
...
onUnmounted(() => {
  ...
  bus.off("reloadUserLevelList", listenToReloadUserLevelList);
});
</script>
<style scoped></style>
