      ...
      <el-row type="flex" justify="center">
        <el-col :span="24">
          <el-form-item label="分类编码" prop="category_code">
            <el-input
              v-model="updateForm.category_code"
              placeholder="分类编码"
              class="yh-input"
              clearable
            />
      ...
      <el-row type="flex" justify="center">
        <el-col :span="24">
          <el-form-item label="分类图片" prop="category_picture_id">
            <div class="yh-input">
              <UploadSinglePicture
                v-model:upload-picture-id="updateForm.category_picture_id"
...
import type { UpdateCategory } from "@/api/laravel-fast-api/v1/business/group/category/category-type";

//定义组件名称
defineOptions({
  name: "UpdateCategoryTreeForm",
});
//定义上传图片的rurl
const uploadPicutreUrl = ref<string>("");
const updateFormRef = ref<FormInstance>();
const updateForm = reactive<UpdateCategory>({
  id: 0,
  category_name: "",
  category_code: "",
...
const updateRules = reactive<FormRules<UpdateCategory>>({
  category_name: [
...
const props = withDefaults(
  defineProps<{
    propsUpdateForm?: UpdateCategory;
  }>(),
  {
    propsUpdateForm: () => ({
      id: 0,
...
const initUpdateForm = () => {
  updateForm.id = 0;
  updateForm.category_name = "";
  updateForm.category_code = "";
  updateForm.category_picture_id = 0;
  updateForm.deep = 1;
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    if (valid) {
      ...
      const result = await CategoryAPI.updateCategory(updateForm);
