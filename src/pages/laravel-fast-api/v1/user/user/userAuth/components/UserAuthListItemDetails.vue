  ...
  <div class="expanded">
    ...
    <div class="content">
      ...
      <div :class="{ show: showIndex == 2, hidden: showIndex != 2 }">
        <UserAuthUserInfo v-if="showComponent === 2" v-model:row-data="rowData" />
      </div>
      <div :class="{ show: showIndex == 3, hidden: showIndex != 3 }">
        <UserAuthQrcode v-if="showComponent === 3" v-model:row-data="rowData" />
      </div>
    </div>
  </div>
</template>
...
import UserAuthUserInfo from "./ListItemDetails/UserAuthUserInfo.vue";
import UserAuthQrcode from "./ListItemDetails/UserAuthQrcode.vue";

//api
import type { UserListItem } from "@/api/laravel-fast-api/v1/user/user/user-type";
//定义组件名称
defineOptions({
  name: "UserAuthListItemDetails",
});
//初始化路由
