 ...
 * @LastEditTime: 2025-12-10 04:59:06
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\user\admin\adminList\components\AdminTableList.vue
...
//工具
//组件
import AdminListItemDetails from "./AdminListItemDetails.vue";
...
//
const beforeAdminForm = defineModel<AdminListItem>("beforeAdminForm", {
  default: () => {
    return {
      id: 0,
      admin_name: "",
...
//选中对象的数组
const multipleSelection = ref<AdminListItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: AdminListItem) => {
...
//定义数据列表
const dataList = defineModel<AdminListItem[]>("dataList", {
  default: () => [],
});
...
`;
// 获取表格实例（需与模板中ref对应）
const adminTable = ref<TableInstance | null>(null);
...
//过滤性别
const filterSex = (value: string, row: AdminListItem) => {
  return row.sex && row.sex == Number(value);
};

//过滤Switch状态
const filterSwitchStatus = (value: string, row: AdminListItem) => {
  return row.switch == value;
};

//编辑
const handleToEditPage = (index: number, row: AdminListItem) => {
  //复杂情景跳转页面
  //console.log(row)
  ...
  dialogType.value = 20;
  beforeAdminForm.value = row;
};

//禁用/解禁
const handleToDisabled = (row: AdminListItem) => {
  // 在外部先声明param变量
  const param = {
    id: row.id,
  ...
  ElMessageBox.confirm(title, text, {
    ...
    .then(async () => {
      try {
        const result = await AdminAPI.disableAdmin(param);
...
//删除
const handleToDelete = (index: number, row: AdminListItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await AdminAPI.deleteAdmin(param);
        ElMessage.success(result.msg);
        emit("update:hanldeUpdateData", true);
      } catch {
        //const err = error as Error;
...
watch(
  ...
  (newValue) => {
    ...
    diffIndex.value.forEach((index) => {
      ...
      if (row) {
        adminTable.value?.toggleRowSelection(row, true);
      }
    });
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        AdminAPI.multipleDeleteAdmin(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
...
watch(
  ...
  (newValue) => {
    ...
    // 确认对话框逻辑不变...
    ElMessageBox.confirm("确认要批量禁用管理员吗？", "确认信息", {
      distinguishCancelAndClose: true,
      confirmButtonText: "禁用",
      cancelButtonText: "取消",
    })
      .then(() => {
        AdminAPI.multipleDisableAdmin(params).then((res) => {
