 ...
 * @LastEditTime: 2025-12-10 04:59:52
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\system\system\otherPlatform\wechat\index.vue
...
<template>
  <div>
    <!-- 页面内容开始 -->
    <YhCard>
      <template #cardTitle>微信配置</template>
      <template #cardNotice>
        <el-tag size="small" type="danger" effect="plain">微信配置微信小程序,小游戏,公众号</el-tag>
      </template>
      <template #cardButton>
        <el-row type="flex" :gutter="20">
          <el-col :span="24">
            <el-button
              ...
              type="primary"
              round
              @click="handleAddSystemWechatConfig()"
            >
              添加微信配置
            </el-button>
          </el-col>
      ...
      <template #cardContent>
        ...
        <YhTableLayout>
          ...
          <template #table>
            ...
            <SystemWechatConfigTableList
              v-model:data-list="dataList"
              v-model:dialog-visible="dialogVisible"
              v-model:dialog-title="dialogTitle"
              v-model:dialog-type="dialogType"
              v-model:before-system-wechat-config-form="beforeSystemWechatConfigForm"
              :loading="loading"
...
//页面级组件
import SystemWechatConfigSearch from "./components/SystemWechatConfigSearch.vue";
import SystemWechatConfigTableList from "./components/SystemWechatConfigTableList.vue";
import CreateSystemWechatConfigForm from "./components/CreateSystemWechatConfigForm.vue";
import UpdateSystemWechatConfigForm from "./components/UpdateSystemWechatConfigForm.vue";
//store
...
//修改用户的容器
const beforeSystemWechatConfigForm = ref<SystemWechatConfigItem>({
  id: 0,
  name: "",
  type: 30,
  appid: "",
...
//监听beforeSystemWechatConfigForm的变化
watch(
  () => beforeSystemWechatConfigForm.value,
  (newValue) => {
...
watch([currentPage, pageSize], () => {
  loading.value = true;
  getSystemWechatConfigList();
});

//修改表单
const propsUpdateForm = ref<UpdateSystemWechatConfig>({
  id: 0,
  name: "",
  type: 30,
  appid: "",
...
//获取列表
const getSystemWechatConfigList = async () => {
  const result = await SystemWechatConfigAPi.getSystemWechatConfig(where.value);
...
//添加
const handleAddSystemWechatConfig = () => {
  dialogTitle.value = "添加微信配置";
  dialogType.value = 10;
