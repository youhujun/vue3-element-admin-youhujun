 ...
 * @LastEditTime: 2025-12-10 04:21:47
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\system\system\otherPlatform\wechat\components\SystemWechatConfigTableList.vue
 * Copyright (C) 2025 youhujun. All rights reserved.
-->
<template>
  <div>
    <el-table
      ref="systemWechatConfigTable"
      v-loading="loading"
    ...
    >
      ...
      <el-table-column label="详情" type="expand" align="center" fixed>
        <template #default="scope">
          <SystemWechatConfigListItemDetails
      ...
      >
        <template #default="scope">
          <el-tag v-show="scope.row.type == 10" type="primary">小程序</el-tag>
          <el-tag v-show="scope.row.type == 20" type="warning">小游戏</el-tag>
          <el-tag v-show="scope.row.type == 30" type="success">公众号</el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" prop="name" label="名称" min-width="300" />
      <el-table-column align="center" prop="appid" label="appid" min-width="300" />
      <el-table-column align="center" prop="appsecret" label="appsecret" min-width="500" />
      <el-table-column
        align="center"
        prop="auth_redirect_url"
        label="授权跳转链接"
        min-width="500"
      />
...
//组件
import SystemWechatConfigListItemDetails from "./SystemWechatConfigListItemDetails.vue";
...
import SystemWechatConfigAPI from "@/api/laravel-fast-api/v1/system/system/otherPlatform/wechat/system-wechat-config-api";

//定义组件
defineOptions({
  name: "SystemWechatConfigTableList",
});
...
//
const beforeSystemWechatConfigForm = defineModel<SystemWechatConfigItem>(
...
//选中对象的数组
const multipleSelection = ref<SystemWechatConfigItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: SystemWechatConfigItem) => {
...
//定义数据列表
const dataList = defineModel<SystemWechatConfigItem[]>("dataList", {
  default: () => [],
});
...
`;
// 获取表格实例（需与模板中ref对应）
const systemWechatConfigTable = ref<TableInstance | null>(null);
//类型
const filterType = (value: string, row: SystemWechatConfigItem) => {
  return Number(row.type) == Number(value);
};
//编辑
const handleToEditPage = (index: number, row: SystemWechatConfigItem) => {
  //复杂情景跳转页面
  //console.log(row)
  //  let encryptString =  encryptData(row);
  //  router.push({
  //   name: 'editSystemWechatConfig',
  ...
  dialogType.value = 20;
  beforeSystemWechatConfigForm.value = row;
};

//删除
const handleToDelete = (index: number, row: SystemWechatConfigItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await SystemWechatConfigAPI.deleteSystemWechatConfig(param);
...
watch(
  ...
  (newValue) => {
    ...
    // 执行反选操作
    systemWechatConfigTable.value?.clearSelection();
    diffIndex.value.forEach((index) => {
      const row = dataList.value[index];
      if (row) {
        systemWechatConfigTable.value?.toggleRowSelection(row, true);
      }
    });
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        SystemWechatConfigAPI.multipleDeleteSystemWechatConfig(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
