<template>
  <div>
    ...
    >
      ...
      <el-row v-show="updateForm.type == 30" type="flex" justify="center">
        <el-col :span="24">
          <el-form-item label="授权地址" prop="auth_redirect_url">
            <el-input
              v-model="updateForm.auth_redirect_url"
              placeholder="请输入授权地址"
              clearable
              class="yh-input"
            />
      ...
      <el-row type="flex" justify="center" style="text-align: center">
        <el-col :span="12">
          <el-form-item>
            <el-button type="primary" @click="submitForm(updateFormRef)">编辑微信配置</el-button>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item>
...
//api和type
import type { UpdateSystemWechatConfig } from "@/api/laravel-fast-api/v1/system/system/otherPlatform/wechat/system-wechat-config-type";
...
const updateForm = reactive<UpdateSystemWechatConfig>({
  id: 0,
  name: "",
  type: 30,
  appid: "",
...
const updateRules = reactive<FormRules<UpdateSystemWechatConfig>>({
  ...
  auth_redirect_url: [{ type: "string", message: "字符串", trigger: "blur" }],
});
...
const props = withDefaults(
  defineProps<{
    propsUpdateForm?: UpdateSystemWechatConfig;
  }>(),
  {
    propsUpdateForm: () => ({
      id: 0,
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    if (valid) {
      ...
      const result = await SystemWechatConfigAPi.updateSystemWechatConfig(updateForm);
      // 3. 提交成功提示
