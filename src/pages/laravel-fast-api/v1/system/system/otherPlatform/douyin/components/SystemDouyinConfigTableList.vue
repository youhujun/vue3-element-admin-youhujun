 ...
 * @LastEditTime: 2025-12-10 03:58:54
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\system\system\otherPlatform\douyin\components\SystemDouyinConfigTableList.vue
 * Copyright (C) 2025 youhujun. All rights reserved.
-->
<template>
  <div>
    <el-table
      ref="systemDouyinConfigTable"
      v-loading="loading"
    ...
    >
      ...
      <el-table-column label="详情" type="expand" align="center" fixed>
        <template #default="scope">
          <SystemDouyinConfigListItemDetails
...
//组件
import SystemDouyinConfigListItemDetails from "./SystemDouyinConfigListItemDetails.vue";

//api
import type { SystemDouyinConfigItem } from "@/api/laravel-fast-api/v1/system/system/otherPlatform/douyin/system-douyin-config-type";
import SystemDouyinConfigAPI from "@/api/laravel-fast-api/v1/system/system/otherPlatform/douyin/system-douyin-config-api";

//定义组件
defineOptions({
  name: "SystemDouyinConfigTableList",
});
...
//
const beforeSystemDouyinConfigForm = defineModel<SystemDouyinConfigItem>(
  "beforeSystemDouyinConfigForm",
  {
    default: () => {
      return {
        id: 0,
...
//选中对象的数组
const multipleSelection = ref<SystemDouyinConfigItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: SystemDouyinConfigItem) => {
...
//定义数据列表
const dataList = defineModel<SystemDouyinConfigItem[]>("dataList", {
  default: () => [],
});
...
`;
// 获取表格实例（需与模板中ref对应）
const systemDouyinConfigTable = ref<TableInstance | null>(null);

//类型
const filterType = (value: string, row: SystemDouyinConfigItem) => {
  return Number(row.type) == Number(value);
};
//编辑
const handleToEditPage = (index: number, row: SystemDouyinConfigItem) => {
  //复杂情景跳转页面
  //console.log(row)
  //  let encryptString =  encryptData(row);
  //  router.push({
  //   name: 'editSystemDouyinConfig',
  //   query: { row: encryptString }  // 注意：query参数建议序列化，避免对象类型问题
  // })
  //简单情景弹框修改
  dialogVisible.value = true;
  dialogTitle.value = "编辑抖音配置";
  dialogType.value = 20;
  beforeSystemDouyinConfigForm.value = row;
};

//删除
const handleToDelete = (index: number, row: SystemDouyinConfigItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await SystemDouyinConfigAPI.deleteSystemDouyinConfig(param);
...
watch(
  ...
  (newValue) => {
    ...
    // 执行反选操作
    systemDouyinConfigTable.value?.clearSelection();
    diffIndex.value.forEach((index) => {
      const row = dataList.value[index];
      if (row) {
        systemDouyinConfigTable.value?.toggleRowSelection(row, true);
      }
    });
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        SystemDouyinConfigAPI.multipleDeleteSystemDouyinConfig(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
