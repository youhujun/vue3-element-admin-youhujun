 ...
 * @LastEditTime: 2025-12-10 04:59:44
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\system\system\voiceConfig\components\VoiceConfigTableList.vue
...
<template>
  <div>
    ...
    >
      ...
      >
        <template #default="scope">
          <el-tag v-show="scope.row.voice_save_type == 10" type="primary">本地</el-tag>
          <el-tag v-show="scope.row.voice_save_type == 20" type="success">云端</el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" prop="voice_title" label="标题" min-width="300" />
      <el-table-column align="center" prop="channle_name" label="渠道名称" min-width="300" />
      <el-table-column align="center" prop="channle_event" label="渠道事件" min-width="300" />
      <el-table-column align="center" prop="note" label="备注" min-width="300" />
      <el-table-column align="center" prop="created_at" label="添加时间" min-width="200" />
...
//组件
import VoiceConfigListItemDetails from "./VoiceConfigListItemDetails.vue";
...
//
const beforeVoiceConfigForm = defineModel<VoiceConfigItem>("beforeVoiceConfigForm", {
  default: () => {
    return {
      id: 0,
      voice_title: "",
...
//选中对象的数组
const multipleSelection = ref<VoiceConfigItem[]>([]);
...
// 存放展开的id, 每次只展开一行
const getRowKeys = (row: VoiceConfigItem) => {
...
//定义数据列表
const dataList = defineModel<VoiceConfigItem[]>("dataList", {
  default: () => [],
});
...
`;
// 获取表格实例（需与模板中ref对应）
const voiceConfigTable = ref<TableInstance | null>(null);
//类型
const filterType = (value: string, row: VoiceConfigItem) => {
  return Number(row.voice_save_type) == Number(value);
};
//编辑
const handleToEditPage = (index: number, row: VoiceConfigItem) => {
  //复杂情景跳转页面
  //console.log(row)
  //  let encryptString =  encryptData(row);
  //  router.push({
  //   name: 'editVoiceConfig',
  //   query: { row: encryptString }  // 注意：query参数建议序列化，避免对象类型问题
  // })
  //简单情景弹框修改
  dialogVisible.value = true;
  dialogTitle.value = "编辑提示配置";
  dialogType.value = 20;
  beforeVoiceConfigForm.value = row;
};

//删除
const handleToDelete = (index: number, row: VoiceConfigItem) => {
  ...
  ElMessageBox.confirm("确认删除吗?", "确认删除", {
    ...
    .then(async () => {
      try {
        const result = await SystemVoiceAPI.deleteVoiceConfig(param);
...
watch(
  ...
  (newValue) => {
    ...
    // 执行反选操作
    voiceConfigTable.value?.clearSelection();
    diffIndex.value.forEach((index) => {
      const row = dataList.value[index];
      if (row) {
        voiceConfigTable.value?.toggleRowSelection(row, true);
      }
    });
...
watch(
  ...
  (newValue) => {
    ...
    ElMessageBox.confirm("确认要批量删除吗？", "确认信息", {
      ...
      .then(() => {
        SystemVoiceAPI.multipleDeleteVoiceConfig(params).then((result) => {
          ElMessage.success(result.msg);
          emit("update:hanldeUpdateData", true);
        });
      })
