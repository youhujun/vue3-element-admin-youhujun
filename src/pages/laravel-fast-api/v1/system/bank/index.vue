 ...
 * @LastEditTime: 2025-12-10 02:50:53
 * @FilePath: d:\wwwroot\Open\Vue\vue3-element-admin-youhujun\src\pages\laravel-fast-api\v1\system\bank\index.vue
...
<template>
  <div>
    <!-- 页面内容开始 -->
    <YhCard>
      <template #cardTitle>银行管理</template>
      <template #cardNotice>
        <el-tag size="small" type="danger" effect="plain">
          提示:添加银行时,设置为默认,在选择时会默认出现在选项中
        </el-tag>
      </template>
      <template #cardButton>
      ...
      <template #cardContent>
        ...
        <YhTableLayout>
          ...
          <template #search>
            ...
            <BankSearch @update:handle-change-search="toSearch"></BankSearch>
          </template>
          <!-- 搜索区结束 -->
          <!-- 批量操作区开始 -->
          <template #batch>
          ...
          <template #table>
            ...
            <BankTableList
              ...
              v-model:before-bank-form="beforeBankForm"
              :loading="loading"
...
import type { UpdateBank, BankItem } from "@/api/laravel-fast-api/v1/system/bank/bank-type";
...
//修改用户的容器
const beforeBankForm = ref<BankItem>({
  id: 0,
  bank_name: "",
  bank_code: "",
  is_default: 10,
...
//修改表单
const propsUpdateForm = ref<UpdateBank>({
  id: 0,
  bank_name: "",
  bank_code: "",
  is_default: 0,
...
const getBankList = async () => {
  const result = await SystemBankAPI.getBank(where.value);
